...
class Gh3220CommandMetaTest {
    ...
    @Test
    fun `version type options follow C firmware algo ver type = 0x12 plus func offset`() {
        val values = Gh3220CommandMeta.VERSION_TYPE_OPTIONS.map { it.value as Int }.toSet()
        assertEquals(
            setOf(0x01, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C),
            values,
        )
    }
