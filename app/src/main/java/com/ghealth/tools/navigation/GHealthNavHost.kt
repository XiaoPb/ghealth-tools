...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun RestartConfigDialog(
    defaultName: String,
    ...
    var scenario by remember { mutableStateOf(defaultScenario) }
    var testRound by remember { mutableStateOf(defaultRound.coerceAtLeast(1).toString()) }
    var scenarioExpanded by remember { mutableStateOf(false) }

    val isValid = testerName.isNotBlank() && (testRound.toIntOrNull() ?: 0) > 0
    ...
    AlertDialog(
            ...
            Column {
                ...
                ExposedDropdownMenuBox(
                    ...
                    ExposedDropdownMenu(
                        ...
                        TestScenario.entries.forEach { s ->
                            DropdownMenuItem(
                ...
                OutlinedTextField(
                    value = testRound,
                    onValueChange = { testRound = it.filter { c -> c.isDigit() } },
                    label = { Text("测试轮次") },
                    singleLine = true,
        ...
        confirmButton = {
            TextButton(
                onClick = {
                    val round = testRound.toIntOrNull() ?: 1
                    if (round > 0 && testerName.isNotBlank()) {
                        onConfirm(TestConfig(testerName = testerName.trim(), scenario = scenario, testRound = round))
                    }
                },
                enabled = isValid
            ) { Text("开始录制") }
