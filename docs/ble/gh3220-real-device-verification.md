...
`ble-gh3220` 已交付 0x08/0x09/0x0A/0x0B/0x2A rawdata 解码与 0x0F 固件升级流程，依据协议文档与设备端 C 源码交叉实现。文档与 C 端源码在以下 5 处存在解释分歧，当前实现按文档优先或计划约定取值，并在 KDoc 中标注「真机抓包未验证」。本清单给出逐项抓包裁决步骤，供硬件就绪后执行：
