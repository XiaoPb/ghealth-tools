...
class FrameBuilder {
    ...
    fun build(
        ...
        val keyBytes = if (isArray) {
            byteArrayOf(key.length.toByte()) + key.toByteArray(Charsets.UTF_8)
        } else {
        ...
        if (hasInvokeIdx) {
            buffer[pos++] = invokeIdx
            crc = (crc + invokeIdx).toByte()
        }
        if (hasFrameIdx && frameIdx != LAST_FRAME_INDEX) {
            buffer[pos++] = frameIdx
            crc = (crc + frameIdx).toByte()
        }
