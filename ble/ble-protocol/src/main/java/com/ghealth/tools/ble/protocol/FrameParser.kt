...
class FrameParser {
    ...
    private fun processByte(byte: Byte): Result<ParseResult?> {
        return when (state) {
            ParseState.FrameHeader -> {
                processFrameHeader(byte)
                Result.success(null)
            }
            ParseState.CheckLength -> {
                frameLen = byte.toInt() and 0xFF
                state = ParseState.CheckTypeKey
                Result.success(null)
            }
            ParseState.CheckTypeKey -> {
                typeKey = TypeKey(byte)
                crc = byte
                frameLen--
                keyExpectedLen = if (typeKey.isArray) 0 else 1
                state = ParseState.CheckKey
                Result.success(null)
            }
    ...
    private fun processKey(byte: Byte): Result<ParseResult?> {
        ...
        if (typeKey.isArray) {
            if (keyData.isEmpty()) {
                if ((byte.toInt() and 0xFF) > MAX_KEY_SIZE - 1) {
                    return Result.failure(ProtocolError.KeyOverMaxSize)
                }
                keyExpectedLen = byte.toInt() and 0xFF
                keyData.add(byte)
            } else {
                keyData.add(byte)
                if (keyData.size > keyExpectedLen) {
                    transitionAfterKey()
                }
            }
        } else {
    ...
    private fun processParam(byte: Byte): Result<ParseResult?> {
        if (frameLen == 0) {
            return processCrc(byte)
        }
        paramData.add(byte)
        crc = (crc + byte).toByte()
        frameLen--
        if (frameLen == 0) {
            state = ParseState.CheckCrc
        }
        return Result.success(null)
    }
    ...
    private fun processCrc(byte: Byte): Result<ParseResult?> {
        if (byte != crc) {
            return Result.failure(ProtocolError.CrcMismatch)
        }

        val key = if (typeKey.isArray && keyData.size > 1) {
            String(keyData.drop(1).toByteArray(), Charsets.UTF_8)
        } else if (keyData.isNotEmpty()) {
            String(keyData.toByteArray(), Charsets.UTF_8)
        } else {
            ""
        }
        ...
        val result = ParseResult(
            key = key,
            param = paramData.toByteArray(),
            isSecure = typeKey.isSecure,
            isFin = typeKey.isFin,
            invokeIdx = frameIndex.invokeIdx,
            frameIdx = frameIndex.frameIdx
        )
        reset()
